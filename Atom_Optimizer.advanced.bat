...
icacls "C:\Program Files\Windows Defender" /grant administrators:F /t /c /q >nul 2>&1
rd /s /q "C:\Program Files\Windows Defender" >nul 2>&1

:: Remove All Windows Bloatware
...
for %%s in (
  "DiagTrack" "dmwappushservice" "MapsBroker" "WSearch" "Fax" "XblGameSave" "XboxNetApiSvc"
  "wlidsvc" "wuauserv" "BITS" "DoSvc" "lfsvc" "ShellHWDetection" "SysMain" "TabletInputService"
  "TrkWks" "UevAgentService" "stisvc" "PcaSvc" "PhoneSvc" "OneSyncSvc" "RetailDemo"
) do (
...
Reg.exe add "HKLM\Software\Microsoft\Windows\Windows Error Reporting\Consent" /v "DefaultConsent" /t REG_DWORD /d "0" /f
Reg.exe add "HKLM\Software\Microsoft\Windows\Windows Error Reporting\Consent" /v "DefaultOverrideBehavior" /t REG_DWORD /d "1" /f
:: Final Cleanup & Restart Explorer
